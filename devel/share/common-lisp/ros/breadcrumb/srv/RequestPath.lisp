...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<RequestPath-request>)))
  ...
  (cl:format cl:nil "# Calculates a path between two points~%# Request:~%#   start:~%#     - Starting point on the grid (in meters)~%#     - Z value is ignored~%#     - In the grids parent frame~%#   end:~%#     - Ending point on the grid (in meters)~%#     - Z value is ignored~%#     - In the grids parent frame~%# Response:~%#   path:~%#     - List of poses returned in the grid's parent frame~%#     - Oriented facing towards the next point~%#     - Empty if no path was found~%~%geometry_msgs/Point start~%geometry_msgs/Point end~%~%~%================================================================================~%MSG: geometry_msgs/Point~%# This contains the position of a point in free space~%float64 x~%float64 y~%float64 z~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'RequestPath-request)))
  "Returns full string definition for message of type 'RequestPath-request"
  (cl:format cl:nil "# Calculates a path between two points~%# Request:~%#   start:~%#     - Starting point on the grid (in meters)~%#     - Z value is ignored~%#     - In the grids parent frame~%#   end:~%#     - Ending point on the grid (in meters)~%#     - Z value is ignored~%#     - In the grids parent frame~%# Response:~%#   path:~%#     - List of poses returned in the grid's parent frame~%#     - Oriented facing towards the next point~%#     - Empty if no path was found~%~%geometry_msgs/Point start~%geometry_msgs/Point end~%~%~%================================================================================~%MSG: geometry_msgs/Point~%# This contains the position of a point in free space~%float64 x~%float64 y~%float64 z~%~%~%"))
